  ...
  $(document).ready(function() {
    ...
    */
    ...
    $('#load-more').click(function() {
      var item1, item2, item3,items,tmp,model, startnum;
	  startnum = $('.item:last').attr("id");
	  firstnum = $('.item').eq(2).attr("id");
	  //alert(firstnum);
	  $.getJSON("hedate.php", {startnum: startnum,firstnum:firstnum}, function(json){
		  var count =json.length-1;
				  tmp = "$()";
		  for(var i=0;i<=count;i++){	
		  
		   tmp = tmp+".add(json["+i+"]['content'])";
		  }

		  //alert(tmp);
      return $container2.isotope('insert', eval(tmp));
			});
    });
    /*
    # =============================================================================
