...
class TagLibCx extends TagLib {
    ...
    protected $tags   =  array(
		...
		'imicmsif'  =>  array('attr'=>'where','level'=>2),
        'elseif'    =>  array('attr'=>'condition','close'=>0),
		'imielse'   =>  array('attr'=>'where','close'=>0),
		...
		'imicmslist'    =>  array('attr'=>'array,foreach,offset,length,key,mod','level'=>3,'alias'=>'iterate'),
	...
	public function _imicmsif($attr,$content) {
        $tag          = $this->parseXmlAttr($attr,'imicmsif');
	...
	public function _imielse($attr,$content) {
        $tag          = $this->parseXmlAttr($attr,'imielse');
        $condition   = $this->parseCondition($tag['where']);
        $parseStr   = '<?php elseif('.$condition.'): ?>';
        return $parseStr;
    }
	...
	public function _imicmslist($attr,$content) {
        static $_iterateParseCache = array();
        //如果已经解析过，则直接返回变量值
        $cacheIterateId = md5($attr.$content);
        if(isset($_iterateParseCache[$cacheIterateId]))
            return $_iterateParseCache[$cacheIterateId];
        $tag      = $this->parseXmlAttr($attr,'imicmslist');
        $name   = $tag['array'];
        $id        = $tag['foreach'];
        $empty  = isset($tag['empty'])?$tag['empty']:'';
        $key     =   !empty($tag['key'])?$tag['key']:'i';
